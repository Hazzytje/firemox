 ...
 */
public class RegisterIndirection extends Modifier {

	/**
	 * Creates a new instance of RegisterIndirection <br>
	 * 
	 * @param context
	 ...
	 *          modifier.
	 */
	public RegisterIndirection(ModifierContext context, int index,
	...
	public int getValue(int oldValue) {
		if (activated) {
			if (next != null) {
				return ((RegisterIndirection) next).getValue(op.process(oldValue,
						rightValue));
			}
			return op.process(oldValue, rightValue);
		}
		if (next != null) {
			return ((RegisterIndirection) next).getValue(oldValue);
		}
		return oldValue;
	}

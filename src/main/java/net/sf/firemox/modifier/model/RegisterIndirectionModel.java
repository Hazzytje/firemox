 ...
 */
public class RegisterIndirectionModel extends ModifierModel {

	/**
	 * Creates a new instance of RegisterIndirectionModel <br>
	 * <ul>
	 ...
	 */
	RegisterIndirectionModel(InputStream inputFile) throws IOException {
		super(inputFile);
		this.index = ExpressionFactory.readNextExpression(inputFile);
		this.rightExpression = ExpressionFactory.readNextExpression(inputFile);
		this.rightExpression.extractTriggeredEvents(linkedEvents,
				SystemCard.instance, True.getInstance());
	...
	protected void addModifierFromModelPriv(Ability ability, MCard target) {
		...
		RegisterIndirection newModifier = new RegisterIndirection(
				new ModifierContext(this, target, ability), index,
				liveUpdate ? rightExpression : new IntValue(rightExpression.getValue(
						ability, target, null)), op);
	 ...
	 */
	protected RegisterIndirectionModel(RegisterIndirectionModel other) {
		super(other);
		index = other.index;
	...
	public RegisterIndirectionModel clone() {
		return new RegisterIndirectionModel(this);
	}
	...
	public String toHtmlString(Ability ability, ContextEventListener context,
		...
		if (other == null) {
			return LanguageManagerMDB.getString("add-registerindirection-modifier-"
	...
	/**
	 * The associated operation of this indirection
	 */

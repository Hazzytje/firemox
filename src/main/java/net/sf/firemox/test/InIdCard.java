 ...
 * @since 0.60
 */
class InIdCard extends TestCard {

	/**
	 * Create an instance of InIdCard by reading a file. Offset's file must
	 * pointing on the first byte of this test <br>
	 * <ul>
	 * Structure of InputStream : Data[size]
	 * <li>idCard to test [Expression]</li>
	 * </ul>
	 * 
	 * @param inputFile
	 ...
	 *           input stream
	 */
	InIdCard(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	protected boolean testCard(Ability ability, MCard tested) {
		return MCard.intersectionIdCard(idCard.getValue(ability, tested, null), on
				.getCard(ability, tested).getIdCard());
	}

	@Override
	public void extractTriggeredEvents(List<MEventListener> res, MCard source,
			Test globalTest) {
		if (idCard.isConstant()) {
			res.add(new ModifiedIdCard(IdZones.PLAY, globalTest, source, idCard));
		}
	}

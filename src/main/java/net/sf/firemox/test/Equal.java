 ...
 * @since 0.54
 */
@XmlTestElement(id = IdTest.EQUAL)
public class Equal extends TestExpr {

	/**
	 * Create an instance of Equal by reading a file. Offset's file must pointing
	 * on the first byte of this test <br>
	 * <ul>
	 ...
	 *           input stream
	 */
	Equal(InputStream inputFile) throws IOException {
		super(inputFile);
	}

	/**
	 * Create a new instance of InfEqual given left and right expression.
	 * 
	 ...
	 */
	private Equal(Expression leftExpression, Expression rightExpression) {
		super(leftExpression, rightExpression);
	}

	@Override
	public Test getConstraintTest(Map<String, Expression> values) {
		return new Equal(leftExpression.getConstraintExpression(values),

 ...
 * @since 0.91
 */
public class PreviousZone extends TestCard {
	 ...
	 *           input stream
	 */
	PreviousZone(InputStream inputFile) throws IOException {
		super(inputFile);
	 ...
	 * @param on
	 */
	public PreviousZone(int idZone, TestOn on) {
		super(on);
		this.idZone = idZone;
	}
	...
	public boolean testCard(Ability ability, MCard tested) {
		// TODO Previous zone is not yet implemented
		MCard card = on.getCard(ability, tested);
		switch (idZone) {
		case IdZones.PLAY_TAPPED:
			return card.getIdZone() == IdZones.PLAY && card.tapped;
		case IdZones.PLAY_UNTAPPED:
			return card.getIdZone() == IdZones.PLAY && !card.tapped;
		default:
			return idZone == card.getIdZone();
		}
	}
	...
	public void extractTriggeredEvents(List<MEventListener> res, MCard source,
		...
		if (idZone >= IdZones.PLAY_TAPPED) {
			...
			res.add(new BecomeTapped(IdZones.PLAY, And.append(new IsTested(on),
					globalTest), source));
			res.add(new BecomeUnTapped(IdZones.PLAY, And.append(new IsTested(on),
					globalTest), source));
		}
		res.add(new MovedCard(IdZones.PLAY, And.append(new IsTested(on),
				new PreviousZone(idZone, on)), globalTest, source));

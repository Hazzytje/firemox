...
import net.sf.firemox.event.ModifiedController;
 ...
 * @since 0.60
 */
class IsController extends TestCard {

	/**
	 * Create an instance of IsController by reading a file. Offset's file must
	 ...
	 *           input stream
	 */
	IsController(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	protected boolean testCard(Ability ability, MCard tested) {
		return on.getCard(ability, tested).getController() == StackManager.PLAYERS[other
	...
	public void extractTriggeredEvents(List<MEventListener> res, MCard source,
			Test globalTest) {
		res.add(new ModifiedController(IdZones.PLAY, globalTest, source));
	}

	@Override
	public Player getOptimizedController(Ability ability,
			ContextEventListener context) {
		if (on == TestOn.TESTED && other.isConstant()) {
			return StackManager.PLAYERS[other.getValue(ability, null, context)];
		}
		return null;
	}

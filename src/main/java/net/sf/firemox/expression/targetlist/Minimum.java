 ...
 */
public class Minimum extends SavedListExpression {

	/**
	 * Creates a new instance of Minimum <br>
	 * <ul>
	 ...
	 *           input stream
	 */
	public Minimum(InputStream inputFile) throws IOException {
		super(inputFile);
		expression = ExpressionFactory.readNextExpression(inputFile);
	}
	...
	public int getValue(Ability ability, Target tested,
		...
		int minimum = 127;
		for (Target target : getList(ability, tested, context)) {
			final int value = expression.getValue(ability, target, context);
			if (value < minimum) {
				minimum = value;
			}
		}

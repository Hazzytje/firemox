 ...
 * @since 0.92
 */
class InputZone extends MessagingAction {

	/**
	 * Create an instance of InputZone by reading a file Offset's file must
	 ...
	 * <li>allowed zones : ListExpression [...]</li>
	 * </ul>
	 * 
	 * @param inputFile
	 ...
	 *           input stream
	 */
	InputZone(InputStream inputFile) throws IOException {
		super(inputFile);
		registerModifier = (ModifyRegister) ActionFactory.readAction(inputFile,
				null);
		allowedZones = new ListExpression(inputFile);
	}
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (controller.isYou()) {
			...
			int[] zones = allowedZones.getList(ability, ability.getCard(), context);

 ...
 * @since 0.82
 */
class IntListRemoveInt extends IntListRemove {

	/**
	 * Create an instance of IntListRemoveTarget by reading a file Offset's file
	 ...
	 */
	IntListRemoveInt(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			StackManager.intList.remove(value);
		} else {
			StackManager.SAVED_INT_LISTS.get(listIndex).remove(value);
		}
		return true;
	}

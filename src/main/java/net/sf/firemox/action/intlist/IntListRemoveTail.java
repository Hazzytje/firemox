 ...
 * @since 0.82
 */
class IntListRemoveTail extends IntList {

	/**
	 * Create an instance of IntListRemoveTail by reading a file Offset's file
	 ...
	 */
	IntListRemoveTail(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			final int last = StackManager.intList.getLastInt();
			StackManager.intList.clear();
			StackManager.intList.addInt(last);
		} else {
			final int last = StackManager.SAVED_INT_LISTS.get(listIndex).getLastInt();
			StackManager.SAVED_INT_LISTS.get(listIndex).clear();
			StackManager.SAVED_INT_LISTS.get(listIndex).addInt(last);
		}
		return true;
	}

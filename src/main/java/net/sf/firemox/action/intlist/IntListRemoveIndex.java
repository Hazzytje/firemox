 ...
 * @since 0.82
 */
class IntListRemoveIndex extends IntListRemove {

	/**
	 * Create an instance of IntListRemoveIndex by reading a file Offset's file
	 ...
	 */
	IntListRemoveIndex(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			if (index < 0) {
				StackManager.intList.removeLastInt();
			} else {
				StackManager.intList.remove(index);
			}
		} else {
			if (index < 0) {

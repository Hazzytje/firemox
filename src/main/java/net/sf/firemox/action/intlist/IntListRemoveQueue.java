 ...
 * @since 0.82
 */
class IntListRemoveQueue extends IntList {

	/**
	 * Create an instance of IntListRemoveQueue by reading a file Offset's file
	 ...
	 */
	IntListRemoveQueue(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			final int first = StackManager.intList.getFirstInt();
			StackManager.intList.clear();
			StackManager.intList.addInt(first);
		} else {
			final int first = StackManager.SAVED_INT_LISTS.get(listIndex)
					.getFirstInt();
			StackManager.SAVED_INT_LISTS.get(listIndex).addInt(first);
		}
		return true;
	}

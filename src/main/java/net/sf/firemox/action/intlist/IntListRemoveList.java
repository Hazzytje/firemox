 ...
 * @since 0.82
 */
class IntListRemoveList extends IntListRemove {

	/**
	 * Create an instance of IntListRemoveList by reading a file Offset's file
	 ...
	 */
	IntListRemoveList(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			StackManager.SAVED_INT_LISTS
					.remove(StackManager.SAVED_INT_LISTS.size() - 1);
		} else {
			StackManager.SAVED_INT_LISTS.remove(listIndex);
		}
		return true;
	}

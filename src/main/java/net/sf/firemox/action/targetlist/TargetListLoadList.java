 ...
 * @since 0.83
 */
class TargetListLoadList extends TargetListLoad {
	 ...
	 */
	TargetListLoadList(InputStream inputFile) throws IOException {
		super(inputFile);
	}
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			list = new ArrayList<Target>(StackManager.SAVED_TARGET_LISTS
					.get(StackManager.SAVED_TARGET_LISTS.size() - 1));
		} else {
			list = new ArrayList<Target>(StackManager.SAVED_TARGET_LISTS
					.get(listIndex));
		}
		StackManager.getTargetListAccess().addAll(list);
		return true;
	}

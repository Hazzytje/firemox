 ...
 * @since 0.82
 */
class TargetListRemoveIndex extends TargetListRemove {

	/**
	 * Create an instance of TargetListRemoveIndex by reading a file Offset's file
	 ...
	 */
	TargetListRemoveIndex(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			if (index < 0) {
				...
				StackManager.getInstance().getTargetedList().remove(index);
			}
		} else {
			if (index < 0) {
	...
	public String toString(Ability ability) {
		return "target-list.remove-index";
	}

	/**
	 * The index of target to remove.
	 */

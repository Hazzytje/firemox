 ...
 * @since 0.83
 */
class TargetListLoadIndex extends TargetListLoad {
	 ...
	 */
	TargetListLoadIndex(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex < 0) {
			StackManager.getTargetListAccess().add(
					StackManager.getTargetListAccess().get(index));
		} else {
			StackManager.getTargetListAccess().add(
					StackManager.SAVED_TARGET_LISTS.get(listIndex).get(index));
		}
		return true;
	}

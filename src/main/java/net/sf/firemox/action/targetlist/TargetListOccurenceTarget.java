 ...
 * @since 0.82
 */
class TargetListOccurenceTarget extends TargetListOccurence {
	 ...
	 */
	TargetListOccurenceTarget(InputStream inputFile) throws IOException {
		super(inputFile);
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (listIndex >= 0) {
			...
			checked = StackManager.getTargetListAccess();
		}
		for (int i = 0; i < checked.size(); i++) {
			if (checked.get(i) == target) {
	...
	public String toString(Ability ability) {
		return "target-list.occurrence-target";
	}

...
/**
 * Prompt to a player the zone to select.
 * 
 * @author <a href="mailto:fabdouglas@users.sourceforge.net">Fabrice Daugan </a>
 * @since 0.92
 */
class InputProperty extends MessagingAction {

	/**
	 * Create an instance of InputProperty by reading a file Offset's file must
	 * pointing on the first byte of this action. <br>
	 * <ul>
	 * Structure of InputStream : Data[size]
	 * <li>controller (idToken) [2]</li>
	 * <li>text to display +'/0' [...]</li>
	 * <li>register modification action [...]</li>
	 * <li>allowed properties : ListExpression [...]</li>
	 * </ul>
	 * 
	 * @param inputFile
	 ...
	 *           input stream
	 */
	InputProperty(InputStream inputFile) throws IOException {
		super(inputFile);
		registerModifier = (ModifyRegister) ActionFactory.readAction(inputFile,
				null);
		allowedProperties = new ListExpression(inputFile);
	}

	@Override
	public final Actiontype getIdAction() {
		return Actiontype.INPUT_ZONE;
	}
	...
	public boolean play(ContextEventListener context, Ability ability) {
		...
		if (controller.isYou()) {
			...
			int[] properties = allowedProperties.getList(ability, ability.getCard(),
					context);
			replayAction(context, ability,
					new net.sf.firemox.ui.wizard.InputProperty(context, ability,
							text, properties));
		} else {
	 ...
	 */
	private final ListExpression allowedProperties;

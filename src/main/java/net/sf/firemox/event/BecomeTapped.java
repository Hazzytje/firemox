 ...
 * @since 0.54
 */
public class BecomeTapped extends TriggeredEvent {

	/**
	 * Create an instance of MEventBecomeTapped by reading a file Offset's file
	 ...
	 *           input stream
	 */
	BecomeTapped(InputStream inputFile, MCard card) throws IOException {
	 ...
	 */
	public BecomeTapped(int idZone, Test test, MCard card) {
		super(idZone, test, card);
	}

	@Override
	public MEventListener clone(MCard card) {
		return new BecomeTapped(idZone, test, card);
	}
	...
	public static void dispatchEvent(MCard tappedCard) {
		for (Ability ability : TRIGGRED_ABILITIES.get(EVENT)) {
			if (ability.isMatching()
					&& ((BecomeTapped) ability.eventComing()).isMatching(ability,
							tappedCard)) {
	 ...
	 * The event type.
	 */
	public static final Event EVENT = Event.BECOMING_TAPPED;

	@Override
	public MEventListener appendOr(MEventListener other) {
		return new BecomeTapped(idZone, Or.append(this.test, other.test), card);
	}

 ...
 * Original filename was MEventModifiedOwner.java
 * 
 ...
 */
public class ModifiedOwner extends TriggeredEvent {

	/**
	 * Creates a new instance of MEventModifiedOwner <br>
	 * <ul>
	 ...
	 *           input stream
	 */
	public ModifiedOwner(InputStream inputFile, MCard card) throws IOException {
	 ...
	 */
	public ModifiedOwner(int idZone, Test test, MCard card) {
	...
	public MEventListener clone(MCard card) {
		return new ModifiedOwner(idZone, test, card);
	}
	 ...
	 */
	public boolean isMatching(int idPlayer) {
		return true;
	}
	 ...
	 * @see #isMatching(int)
	 */
	public static void dispatchEvent(int idPlayer) {
		for (Ability ability : TRIGGRED_ABILITIES.get(EVENT)) {
			if (ability.isMatching()
					&& ((ModifiedOwner) ability.eventComing()).isMatching(idPlayer)) {
				ability.triggerIt(new MContextCardCardIntInt(ability.getCard(),
						idPlayer));
			}
		}
	}
	 ...
	 * The event type.
	 */
	public static final Event EVENT = Event.MODIFIED_OWNER;

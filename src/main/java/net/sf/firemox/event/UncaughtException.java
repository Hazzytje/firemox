 ...
 * @since 0.85
 */
public class UncaughtException extends TriggeredEvent {

	/**
	 * Create an instance of ExceptionEvent by reading a file Offset's file must
	 * pointing on the first byte of this event <br>
	 * <ul>
	 * Structure of InputStream : Data[size]
	 * <li>idZone [1]</li>
	 * <li>test [...]</li>
	 * <li>exception name to catch +'\0' [...]</li>
	 * </ul>
	 * 
	 * @param inputFile
	 ...
	 *           input stream
	 */
	UncaughtException(InputStream inputFile, MCard card) throws IOException {
		super(inputFile, card);
		exceptionName = MToolKit.readString(inputFile);
	}

	/**
	 * Creates a new instance of MEventBecomeTapped specifying all attributes of
	 ...
	 */
	private UncaughtException(int idPlace, Test test, MCard card,
	...
	public MEventListener clone(MCard card) {
		return new UncaughtException(idZone, test, card, exceptionName);
	}
	 ...
	 *          the player throwing this exception.
	 * @param exceptionName
	 *          the exception name to catch
	 * @return true if the current event match with this event
	 */
	public boolean isMatching(Ability ability, Player concernedPlayer,
			String exceptionName) {
		return this.exceptionName.equals(exceptionName)
				&& test(ability, concernedPlayer);
	}
	 ...
	 * @param exceptionName
	 *          the exception name to catch
	 * @see #isMatching(Ability, Player, String)
	 */
	public static void dispatchEvent(Player concernedPlayer, String exceptionName) {
		for (Ability ability : TRIGGRED_ABILITIES.get(EVENT)) {
			if (((UncaughtException) ability.eventComing()).isMatching(ability,
					concernedPlayer, exceptionName)) {
				ability.triggerIt(new MContextCardCardIntInt(concernedPlayer));
			}
		}
	}
	 ...
	 * The event type.
	 */
	public static final Event EVENT = Event.EXCEPTION;

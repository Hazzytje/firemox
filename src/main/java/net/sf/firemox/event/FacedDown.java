 ...
 * @since 0.90
 */
public class FacedDown extends TriggeredEvent {
	 ...
	 *           input stream
	 */
	FacedDown(InputStream inputFile, MCard card) throws IOException {
	 ...
	 */
	public FacedDown(int idZone, Test test, MCard card) {
	...
	public MEventListener clone(MCard card) {
		return new FacedDown(idZone, test, card);
	}
	 ...
	 */
	public boolean isMatching(Ability ability, MCard facedDownCard) {
		return test(ability, facedDownCard);
	}
	 ...
	 */
	public static void dispatchEvent(MCard facedDownCard) {
		for (Ability ability : TRIGGRED_ABILITIES.get(EVENT)) {
			if (ability.isMatching()
					&& ((FacedDown) ability.eventComing()).isMatching(ability,
							facedDownCard)) {
				ability.triggerIt(new MContextCardCardIntInt(facedDownCard));
			}
		}
	}
	 ...
	 * The event type.
	 */
	public static final Event EVENT = Event.FACED_DOWN;
	...
	public MEventListener appendOr(MEventListener other) {
		return new FacedDown(idZone, Or.append(this.test, other.test), card);
	}
}
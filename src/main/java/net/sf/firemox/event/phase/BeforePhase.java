 ...
 * @since 0.54
 */
public class BeforePhase extends PhaseEvent {

	/**
	 * Create an instance of MEventBeforePhase by reading a file Offset's file
	 ...
	 *           input stream
	 */
	public BeforePhase(InputStream inputFile, MCard card) throws IOException {
		super(inputFile, card);
	}

	/**
	 * Creates a new instance of MEventBeforePhase specifying all attributes of
	 ...
	 *          the filter.
	 */
	protected BeforePhase(int idPlace, Test test, MCard card, Expression idPhase,
			PhaseFilter phaseFilter) {
		super(idPlace, test, card, idPhase, phaseFilter);
	}
	 ...
	 * for this event, ONLY the first replacement ability found is picked and
	 * played.
	 * 
	...
	public static boolean tryAction(int currentIdPhase) {
		...
		for (Priority priority : Priority.values()) {
			for (ReplacementAbility ability : map.get(priority)) {
				if (((BeforePhase) ability.eventComing()).isMatching(ability)) {
	...
	public MEventListener clone(MCard card) {
		return new BeforePhase(idZone, test, card, idPhase, phaseFilter);
	}
	 ...
	 * The event type.
	 */
	public static final Event EVENT = Event.BEFORE_PHASE;

	@Override
	public String toHtmlString(Ability ability, ContextEventListener context) {
		return LanguageManagerMDB.getString("event-bp", LanguageManagerMDB
	...
	public MEventListener appendOr(MEventListener other) {
		if (((PhaseEvent) other).idPhase == idPhase) {
			return new BeforePhase(idZone, Or.append(this.test, other.test), card,
					idPhase, phaseFilter);
		}
		return null;
	}

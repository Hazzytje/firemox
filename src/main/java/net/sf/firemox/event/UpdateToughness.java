...
/**
 * When this event is raised. The attached test is applied by default on the
 * card to update. The 'context.card' object can be used in the actions to
 * acceed to the card to update.
 * 
 * @author <a href="mailto:fabdouglas@users.sourceforge.net">Fabrice Daugan </a>
 * @since 0.54
 */
public class UpdateToughness extends TriggeredEvent {

	/**
	 * Create an instance of UpdateToughness by reading a file Offset's file must
	 ...
	 *           input stream
	 */
	UpdateToughness(InputStream inputFile, MCard card) throws IOException {
		super(inputFile, card);
	}

	/**
	 * Creates a new instance of UpdateToughness specifying all attributes of this
	 ...
	 */
	private UpdateToughness(int idPlace, Test test, MCard card) {
	...
	public MEventListener clone(MCard card) {
		return new UpdateToughness(idZone, test, card);
	}
	 ...
	 */
	public boolean isMatching(Ability ability, MCard toUpdate) {
		return test(ability, toUpdate);
	}
	 ...
	 */
	public static void dispatchEvent(MCard toUpdate) {
		for (Ability ability : TRIGGRED_ABILITIES.get(EVENT)) {
			if (((UpdateToughness) ability.eventComing()).isMatching(ability,
					toUpdate)) {
				ability.triggerIt(new MContextTarget(toUpdate));
			}
		}
	}
	 ...
	 * The event type.
	 */
	public static final Event EVENT = Event.UPDATE_TOUGHNESS;

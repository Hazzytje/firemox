 ...
 * @since 0.82
 */
public class Idcardmodifier implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		ModifierType.IDCARD_MODIFIER.serialize(out);
		XmlModifier.buildMdbModifier(node, out);

		// the operation to apply
		IdOperation idOperation = IdOperation.OR;
		if (node.getAttribute("operation") != null) {
			idOperation = XmlTools.getOperation(node.getAttribute("operation"));
		}
		idOperation.serialize(out);

		// the modified idcard
		if (idOperation == IdOperation.CLEAR) {
			IdOperation.INT_VALUE.serialize(out);
			MToolKit.writeInt16(out, 0);
		} else
			XmlTools.writeAttrOptions(node, "idcard", out);
		return 0;
	}

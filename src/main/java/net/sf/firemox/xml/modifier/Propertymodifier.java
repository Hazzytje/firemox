 ...
 * @since 0.82
 */
public class Propertymodifier implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		ModifierType.PROPERTY_MODIFIER.serialize(out);
		XmlModifier.buildMdbModifier(node, out);

		// the hasNOT tag
		out
				.write("remove".equalsIgnoreCase(node.getAttribute("operation")) ? 0
						: 1);

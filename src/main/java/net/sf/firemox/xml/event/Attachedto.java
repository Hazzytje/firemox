 ...
 * @since 0.82
 */
public class Attachedto implements XmlToMDB {
	 ...
	 * <li>idEvent [1]</li>
	 * <li>idZone [1]</li>
	 * <li>test on the card is going to be attached to X [...]</li>
	 * <li>test on the card X [...]</li>
	 * </ul>
	 * 
	 * @param node
	 ...
	 * @see net.sf.firemox.event.AttachedEvent
	 * @throws IOException
	 */
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		// write the idEvent
		Event.ATTACHED_TO.write(out);
		XmlEvent.buildMdbTriggered(node, out);

		// write the test
		XmlTest.getTest("test").buildMdb(node.get("attached-test"), out);
		// write the test
		XmlTest.getTest("test").buildMdb(node.get("by-test"), out);
		return 0;
	}

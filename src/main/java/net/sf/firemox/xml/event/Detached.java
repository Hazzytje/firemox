 ...
 * @since 0.82
 */
public class Detached implements XmlToMDB {
	 ...
	 * @see net.sf.firemox.event.Detached
	 * @throws IOException
	 */
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		// write the idEvent
		Event.DETACHED_CARD.write(out);
		XmlEvent.buildMdbTriggered(node, out);

		// write the tests
		XmlTest.getTest("test").buildMdb(node.get("detached-test"), out);
		XmlTest.getTest("test").buildMdb(node.get("from-test"), out);
		return 0;
	}

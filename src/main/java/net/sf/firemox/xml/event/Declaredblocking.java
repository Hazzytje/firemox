 ...
 * @since 0.82
 */
public class Declaredblocking implements XmlToMDB {
	 ...
	 * @see net.sf.firemox.event.DeclaredBlocking
	 * @throws IOException
	 */
	public final int buildMdb(XmlParser.Node node, OutputStream out)
		...
		// write the test
		XmlTest.getTest("test").buildMdb(node.get("blocking-test"), out);
		XmlTest.getTest("test").buildMdb(node.get("attacking-test"), out);
		return 0;
	}

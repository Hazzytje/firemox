 ...
 * @since 0.82
 */
public class Isthis implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		if (node.getAttribute("player") != null) {
			// this is a test on player
			XmlTest.buildMdbPlayerplayer(node, out, IdTest.ISME_PLAYER);
		} else {
			// this is a test on card
			XmlTest.buildMdbCard(node, out, IdTest.IS_TESTED);

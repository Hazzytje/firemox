 ...
 * @since 0.93
 */
public class Replacetested implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		IdTest.REPLACE_TESTED.serialize(out);

		TestOn.serialize(out, node.getAttribute("by"));
		return XmlTest.getTest("test").buildMdb(node, out);
	}

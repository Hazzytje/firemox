 ...
 * @since 0.82
 */
public class Database implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		XmlTest.buildMdbCard(node, out, IdTest.DATABASE);
		MToolKit.writeString(out, node.getAttribute("property"));
		MToolKit.writeString(out, node.getAttribute("value"));
		return 0;
	}

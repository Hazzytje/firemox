 ...
 * @since 0.82
 */
public class Hasability implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		XmlTest.buildMdbCard(node, out, IdTest.HAS_ABILITY);
		String name = node.getAttribute("name");
		if (name == null || name.length() == 0) {
			System.out
					.println("\t>> In has-ability test, 'name' attribute must not be empty");
		}
		MToolKit.writeString(out, name);
		return 0;
	}

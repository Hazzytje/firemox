...
/**
 * Expression returning the result of an operation on properties of a card.
 * 
 * @author <a href="mailto:fabdouglas@users.sourceforge.net">Fabrice Daugan </a>
 * @since 0.95
 */
public class CardProperties implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		IdOperation.CARD_PROPERTIES.serialize(out);
		CardPropertiesOperation.valueOfXsd(node.getAttribute("operation"))
				.serialize(out);
		XmlTools.writeTestOn(out, node.getAttribute("card"));
		XmlTools.writeTestOn(out, node.getAttribute("card2"));
		XmlTools.writeAttrOptions(node, "lower", out);
		XmlTools.writeAttrOptions(node, "higher", out);
		return 0;
	}

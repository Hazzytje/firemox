 ...
 * @since 0.90
 */
public class Privateobject implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		// write the type
		Actiontype.OBJECT_MAP.write(out);
		...
		case ADD:
		case SAVE:
			if (node.getAttribute("object") == null) {
				throw new InternalError(
						"\t>>'object' attribute is required for 'add' operation");
			}
			TestOn.serialize(out, node.getAttribute("object"));
			if (node.getAttribute("object-name") == null) {
				throw new InternalError(
						"\t>>'object-name' attribute is required for 'add' operation");
			}
		...
		case REMOVE:
			if (node.getAttribute("object") == null) {
				throw new InternalError(
						"\t>>'object' attribute is required for 'remove' operation");
			}
			TestOn.serialize(out, node.getAttribute("object"));

			if (node.getAttribute("object-name") == null) {
				throw new InternalError(
						"\t>>'object-name' attribute is required for 'remove' operation");
			}
			MToolKit.writeString(out, node.getAttribute("object-name"));
			break;
		default:

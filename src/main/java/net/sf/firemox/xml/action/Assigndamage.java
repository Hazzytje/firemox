 ...
 * @since 0.82
 */
public class Assigndamage implements XmlToMDB {
	...
	public int buildMdb(XmlParser.Node node, OutputStream out) throws IOException {
		final boolean fromToMode = node.getAttribute("from") != null
				|| node.getAttribute("to") != null;
		if (fromToMode) {
			// From --> to rule
			XmlAction.buildMdb(Actiontype.ASSIGN_DAMAGE_FROM_TO, node, out);
		} else {
			// all targets are damaged
			XmlAction.buildMdb(Actiontype.ASSIGN_DAMAGE_TARGET, node, out);
		}
		XmlTools.writeAttrOptions(node, "value", out);
		XmlTools.writeAttrOptionsDefault(node, "type", out, "damage-normal");
		if (fromToMode) {
			if (node.getAttribute("from") == null) {
				XmlConfiguration
						.error("In 'assign-damage' action, 'from' attribute must be specified with 'to' attribute"
								+ node);
			...
			if (node.getAttribute("to") == null) {
				XmlConfiguration
						.error("In 'assign-damage' action, 'to' attribute must be specified with 'from' attribute"
								+ node);
				TestOn.THIS.serialize(out);
			} else {

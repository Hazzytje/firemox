 ...
 * @since 0.82
 */
public class Moveplayercard implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		XmlAction.buildMdb(Actiontype.MOVE_PLAYER_CARD, node, out);
		TestOn.serialize(out, node.getAttribute("controller"));
		out.write(XmlTools.getZone(node.getAttribute("from")));
		out.write(XmlTools.getZone(node.getAttribute("to")));
		MToolKit.writeInt16(out, XmlTools
				.getPosition(node.getAttribute("position")));
		return 1;
	}

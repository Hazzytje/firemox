 ...
 * @since 0.82
 */
public class Setidcard implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		XmlAction.buildMdb(Actiontype.SET_ID_CARD, node, out);
		XmlTools.writeAttrOptions(node, node.getAttribute("idcard"), out);
		return 1;
	}

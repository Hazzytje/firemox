 ...
 * @since 0.82
 */
public class Inputnumber extends MessageAction implements XmlToMDB {
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		buildMdb(node, out, Actiontype.INPUT_NUMBER);
		XmlTools.writeAttrOptions(node, "minimum", out);
		XmlTools.writeAttrOptions(node, "maximum", out);
		XmlTools.writeAttrOptionsDefault(node, "default", out, "%maximum");
		if ("false".equals(node.getAttribute("strict-max"))) {
			out.write('\0');
		} else {
			out.write('\1');
		}

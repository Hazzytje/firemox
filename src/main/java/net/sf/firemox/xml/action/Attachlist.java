 ...
 * @since 0.82
 */
public class Attachlist implements XmlToMDB {
	...
	public int buildMdb(XmlParser.Node node, OutputStream out) throws IOException {
		XmlAction.buildMdb(Actiontype.ATTACH_LIST, node, out);
		XmlTest.getTest("test").buildMdb(node.get("valid-attachment"), out);
		return 1;
	}
}

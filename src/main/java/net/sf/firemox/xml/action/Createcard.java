 ...
 * @since 0.82
 */
public class Createcard implements XmlToMDB {
	...
	public int buildMdb(XmlParser.Node node, OutputStream out) throws IOException {
		XmlAction.buildMdb(Actiontype.CREATE_CARD, node, out);
		XmlTbs.cardPager.enable(false);
		XmlTbs.getTbsComponent("card").buildMdb(node.get("card"), out);
		XmlTbs.cardPager.enable(true);
		return 1;
	}

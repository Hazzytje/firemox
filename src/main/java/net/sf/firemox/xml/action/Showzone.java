 ...
 * @since 0.82
 */
public class Showzone implements XmlToMDB {
	 ...
	 * <li>to him=0,to opponent=1 [1]</li>
	 * </ul>
	 * 
	 * @param node
	...
	public final int buildMdb(XmlParser.Node node, OutputStream out)
			throws IOException {
		XmlAction.buildMdb(Actiontype.ZONE_VISIBILITY, node, out);

		out.write(XmlTools.getZone(node.getAttribute("zone")));
		out.write("show-zone".equals(node.getTag()) ? 1 : 0);
		VisibilityChange.valueOf(node.getAttribute("for")).serialize(out);
		return 1;
	}
